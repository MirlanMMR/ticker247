...
import com.mirlanmamytov.ticker247.youtube.YouTubeWatcher
...
class TickerForegroundService : Service() {
            ...
            serviceScope.launch(Dispatchers.IO) {
                try {
                    ...
                    withContext(Dispatchers.Main) {
                        DataBridge.newsItems.clear()
                        DataBridge.newsItems.addAll(newItems)
        ...
        serviceScope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    ...
                    // 4. YouTube личные каналы — проверяем новые видео
                    try {
                        YouTubeWatcher.checkNewVideos(this@TickerForegroundService)
                    } catch (e: Exception) { Log.e("Ticker247", "YouTube watcher: ${e.message}") }
                    ...
                    withContext(Dispatchers.Main) {
                        DataBridge.currentNewsList.clear()
                        DataBridge.currentNewsList.addAll(tickerItems)
                        DataBridge.newsItems.clear()
                        DataBridge.newsItems.addAll(allItems)
                        DataBridge.tickerLine = tickerItems.joinToString("  ·  ")
                    }
                } catch (e: Exception) {
                    Log.e("Ticker247", "Fetch error: ${e.message}")
                }
                delay(5 * 60_000L)
            }
        }
    ...
    private suspend fun fetchData() = coroutineScope {
        ...
        withContext(Dispatchers.Main) {
            ...
            DataBridge.currentNewsList.addAll(tickerItems)
            DataBridge.newsItems.clear()
            DataBridge.newsItems.addAll(newsItems)
            DataBridge.tickerLine = tickerItems.joinToString("  ·  ")
        }
    }
