...
object OgImageFetcher {
    ...
    private val CATEGORY_IMAGES = mapOf(
    ...
    suspend fun fetch(articleUrl: String, title: String = "", category: String = ""): String? {
        ...
        return withContext(Dispatchers.IO) {
            // ── Шаг 1: og:image со страницы статьи ──────────────────────────
            val ogImage = if (articleUrl.isNotBlank()) fetchOgImage(articleUrl) else null
            if (ogImage != null) {
                cache[cacheKey] = ogImage
                return@withContext ogImage
            }

            // ── Шаг 2: Wikipedia — ищем фото по заголовку статьи ────────────
            if (title.isNotBlank()) {
                val wikiImage = fetchWikipediaImage(title)
                if (wikiImage != null) {
                    cache[cacheKey] = wikiImage
                    return@withContext wikiImage
                }
            }

            // ── Шаг 3: Категорийный fallback ─────────────────────────────────
            val fallback = CATEGORY_IMAGES[category] ?: CATEGORY_IMAGES["NEWS"]!!
